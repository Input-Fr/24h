...
static char *convert_to_canonical(const char* curpath)
{
    ...
    while (token)
    {
        if (strcmp(token, "") != 0 && strcmp(token, ".") != 0) // dot or / skipped
        {
            if (strcmp(token, "..") == 0)
            {
...
int cmd_cd(char *arg) // following SCL algorithm
{
        ...
        strlen(curpath) < pmax || strlen(arg) >= pmax)
    {
        if (pwd != NULL)
        {
            ...
            snprintf(pwd_slash, sizeof(pwd_slash), "%s%s", pwd, pwd[pwd_len - 1] == '/' ? "" : "/");
            if (strncmp(curpath, pwd_slash, strlen(pwd_slash)) == 0) // pwd in curpath ?
            {
                size_t prefix_len = strlen(pwd_slash);
                memmove(curpath, curpath + prefix_len, strlen(curpath) - prefix_len + 1);
            }
        }
    }
    // --------------
